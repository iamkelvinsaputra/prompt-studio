...
enum class StudioCategory(val title: String, val description: String, val modules: List<EditorModule>, val sections: Set<String>) {
    Subject("Subject", "Find the person behind the image.", listOf(EditorModule.Identity, EditorModule.Expression, EditorModule.Hair, EditorModule.Face, EditorModule.Body, EditorModule.Role, EditorModule.Personality, EditorModule.Contradiction, EditorModule.VisualThesis), setOf("CHARACTER IDENTITY", "SUBJECT", "BODY", "FACE", "HAIR", "EXPRESSION", "ROLE", "PERSONALITY READ", "INNER CONTRADICTION", "CORE VISUAL THESIS")),
    Appearance("Appearance", "Build an outfit with a distinct silhouette.", listOf(EditorModule.Costume, EditorModule.Accessories, EditorModule.ShapeLanguage, EditorModule.Prop), setOf("APPEARANCE MOTION", "COSTUME", "ACCESSORIES", "SHAPE LANGUAGE", "PROP / WEAPON / TOOL")),
    Pose("Pose", "Find the gesture. Give it intention.", listOf(EditorModule.Pose, EditorModule.Gaze), setOf("POSE", "GAZE / HEAD DIRECTION")),
    Camera("Camera", "Choose what we see, and how it fills the frame.", listOf(EditorModule.Composition), setOf("COMPOSITION")),
    ...
    Color("Color", "Set the palette and choose where color leads.", listOf(EditorModule.AccentColor), setOf("COLOR DIRECTION", "ACCENT COLOR POLICY")),
    Effects("Effects", "Add movement, atmosphere, or a signature effect.", listOf(EditorModule.Power), setOf("EFFECTS", "SUPERNATURAL SIGNATURE")),
...
fun StudioCategory.summary(p: CharacterProject): String = when (this) {
    ...
    StudioCategory.Environment -> p.environment.worldContextHint.ifBlank { p.environment.category?.label ?: p.environment.abstractionLevel?.wording ?: "Minimal" }
    StudioCategory.Lighting -> p.lighting.preset.ifBlank { listOfNotNull(p.lighting.quality.takeIf { it.isNotBlank() }, p.lighting.lightDirection?.label, p.lighting.sourceQuality?.wording).joinToString(" · ").ifBlank { p.lighting.direction.ifBlank { "Soft daylight" } } }
    StudioCategory.Style -> p.artStyle.preset?.label ?: if (p.artStyle.manualMode) "Custom style" else p.style.name
    StudioCategory.Color -> p.colorDirection.description.ifBlank { p.colorDirection.colors.joinToString(" · ") { it.hex }.ifBlank { p.colorAccents.accentColor?.wording ?: "Natural balanced" } }
    StudioCategory.Effects -> p.effects.items.joinToString(" · ") { it.name }.ifBlank { p.effects.customDescription.ifBlank { p.powerSignature.manifestation.ifBlank { "None" } } }
