...
@Composable
fun GenerationPanel(project: CharacterProject, controller: GenerationController, credentials: CredentialStore, selection: GenerationSelection, onMessage: (String) -> Unit) {
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        ImageModels.all.filter { it.provider == provider }.forEach { option ->
            FilterChip(model == option, { selection.model = option }, label = { Text(option.displayName) })
        }
        if (keyConfigured) {
        ...
        Button(enabled = draftKey.isNotBlank(), onClick = {
            try { credentials.set(provider, draftKey, rememberKey); draftKey = ""; keyConfigured = true }
        ...
        Text(if (effective == null) "Enter a numeric width:height ratio in Output to generate."
        ...
        when (val status = state.status) {
            is GenerationState.Generating -> {
                ...
                Text("Generating ${status.metadata.project.name} with ${status.metadata.request.model}…")
